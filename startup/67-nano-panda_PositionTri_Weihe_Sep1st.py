...
def scan_and_fly_2dpd(detectors, xcenter, xrange, xnum, ystart, ystop, ynum, dwell, *,
    ...
    for det_name in ("merlin2", "eiger2", "eiger_mobile", "eiger3"):
        if det_name in dets_by_name:
            ...
            if det_name == "eiger3":
                ...
                #  data representation for short and long exposures (just an assumption).
                #dpc.hdf5.warmup(acquire_time=acquire_time)
                if acquire_time < 1: #Weihe_7/31: 0.00667=>1, to fix image_bitdepth at 16 for initial testing
                    image_bitdepth = 16
                else:
                    image_bitdepth = 32
                if 'UInt'+str(image_bitdepth) != dpc.hdf5.data_type.get():
    ...
    for d in panda_flyer.detectors:
        if d.name == 'eiger2' or d.name == 'eiger3' or  d.name == 'eiger_mobile':
            yield from bps.mov(d.fly_next, True)
            yield from bps.mov(d.internal_trigger, False) #Weihe@7/31 changed from 'True' to 'False'
    #@subs_decorator(livepopup)
    @subs_decorator({'start': at_scan})
    @subs_decorator({'stop': finalize_scan})
    @ts_monitor_dec([roi_pv])
    ...
    def plan():
        ...
        while counter<num_total:
            ...
            yield from bps.sleep(3) #Weihe_7/31: 0.3=>3 to accommodate the overrun and return scan, need to be even longer for very large scale
...
def pt_fly2dcontpd(dets, motor1, scan_start1, scan_end1, num1, motor2, scan_start2, scan_end2, num2, exposure_time, pos_return = True, apply_tomo_drift = False, tomo_angle = None, auto_rescan = False,position_supersample= 1, scan_downward = True, **kwargs):
    """
    Relative scan
    """
    do_scan = True
    while do_scan:
        ...
        try:
            ...
            fg_volt = 1.6*range1/10.0 #Weihe_7/31: 6.0=>10.0 to match the 100 um range scanner, 1=>1.6 to add 30% overrun at each end
            fg_offset = -center1/10.0 #Weihe_7/31: 6.0=>10.0 to match the 100 um range scanner
            fg_freq = 1/(1.0*num1*exposure_time) #Weihe_7/31: 2.0=>1.0 for raster scan
            ...
            yield from bps.abs_set(panda2.pulse1.width,(exposure_time-dead_time)/2.0/position_supersample/10) #added /10, positin _sumpersample use '1', just check the position at the start moment
            yield from bps.abs_set(panda2.pulse1.width_units,'s')
            yield from bps.abs_set(panda2.pulse1.step,(exposure_time-dead_time)/position_supersample/10) #added /10
            yield from bps.abs_set(panda2.pulse1.step_units,'s')
            yield from bps.abs_set(panda2.pulse1.delay,(exposure_time-dead_time)/2.0/position_supersample/10) #added /10
            ...
            yield from scan_and_fly_2dpd(dets, center1, range1, num1, start2, end2, num2, exposure_time, dead_time = dead_time, delta = 5, position_supersample = position_supersample, **kwargs) #Weihe_7/31: added 'delta = 5'
